...
import { X, User, Calendar, Mail, Phone, Activity, Clock, UserPlus, Edit, Plus } from 'lucide-react'
          ...
          <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${ageCategory.color}`}>
                  {ageCategory.label}
                </span>
              </div>
            </div>
            ...
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-800">{surgeries.length}</p>
                </div>
            ...
            <div className="space-y-3">
              {loadingSurgeries ? (
                ...
                surgeries.map((surgery) => (
                  <div key={surgery.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <h5 className="font-medium text-gray-900">{surgery.type}</h5>
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(surgery.status)}`}>
                            {surgery.status.replace('_', ' ')}
                          </span>
                        </div>
                        <p className="text-sm text-gray-600">
                          {new Date(surgery.scheduledAt).toLocaleDateString('en-US', { 
                            weekday: 'short',
                            year: 'numeric', 
                            month: 'short', 
                            day: 'numeric',
                        ...
                        <p className="text-sm text-gray-600">Surgeon: {surgery.surgeon.name}</p>
                        {surgery.notes && (
                          <p className="text-xs text-gray-500 mt-1 italic">"{surgery.notes}"</p>
                        )}
                      </div>
                      <Button
                        variant="ghost"
