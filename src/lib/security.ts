...
export async function logAuditEvent(
  ...
  if (!context) {
    console.error("CRITICAL: Cannot log audit event without security context");
    return;
  }
...
const ENCRYPTION_KEY =
  process.env.PHI_ENCRYPTION_KEY || "dev-key-32-chars-long-for-aes-256";
...
export function decryptPHI(encryptedData: {
  encrypted: string;
  iv: string;
  tag: string;
}): string {
