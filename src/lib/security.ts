...
export function checkRateLimit(ipAddress: string, maxRequests: number = 100, windowMs: number = 15 * 60 * 1000): boolean {
  const now = Date.now()
  const key = ipAddress
...
export function validatePHIInput(data: any, requiredFields: string[]): { valid: boolean; errors: string[] } {
  const errors: string[] = []
